...
(
    ...
    NULL
    COMMENT
    '关联上传任务ID（普通上传时有效；秒传或离线导入时可为空）',
    file_name
    VARCHAR
(
    128
) NOT NULL COMMENT '最终存储文件名（含扩展名）',
...
(
    256
) NOT NULL COMMENT '存储路径或对象存储Key',
    storage_key VARCHAR
(
    64
) NOT NULL COMMENT '实际存储节点Key',
    file_url VARCHAR
(
    512
) NULL COMMENT '文件访问URL（CDN地址）',
    file_size BIGINT DEFAULT 0 NOT NULL COMMENT '文件大小（字节）',
    file_type VARCHAR
(
    32
) NULL COMMENT '文件类型：image/video/document/audio/other',
    mime_type VARCHAR
(
    64
) NULL COMMENT 'MIME类型',
...
(
    16
) NULL COMMENT '文件扩展名（小写）',
    md5 CHAR
(
    32
) NOT NULL COMMENT '32位小写MD5值（用于全局去重与校验）',
...
(
    32
) NULL COMMENT '业务分类：avatar/attachment/comment/temp等',
...
(
    ...
    INDEX idx_storage_key
(
    storage_key
) COMMENT '按存储节点查询文件',
    INDEX idx_reference_count
(
    reference_count
    DESC
) COMMENT '热门文件排序'
...
(
    32
) NULL COMMENT '引用类型：avatar/article_attachment/comment_image/temp',
    reference_id BIGINT NULL COMMENT '引用记录ID',
    category VARCHAR
(
    32
) NULL COMMENT '业务分类：avatar/attachment/comment/temp',
...
(
    3
) NOT NULL COMMENT '过期时间（自动清理未完成任务）',
    error_code VARCHAR
(
    32
) NULL COMMENT '错误码（如: FILE_TOO_LARGE, CHUNK_MD5_MISMATCH）',
...
(
    ...
    INDEX idx_referenced_file
(
    referenced_file_id
) COMMENT '引用关系查询',
    INDEX idx_status_expire
(
    task_status,
    expire_time
) COMMENT '过期任务清理',
    INDEX idx_quick_upload_time
(
    is_quick_upload,
    quick_upload_time
) COMMENT '秒传统计'
...
(
    ...
    UNIQUE KEY uk_task_chunk
(
    upload_task_id,
    chunk_number
) COMMENT '任务内分片唯一',
    INDEX idx_upload_status
(
    upload_task_id,
    upload_status
) COMMENT '分片进度查询',
    INDEX idx_chunk_number
(
    upload_task_id,
    chunk_number
) COMMENT '按序号快速定位'
...
(
    ...
    UNIQUE KEY uk_file_reference
(
    file_id,
    user_id,
    reference_type,
    reference_id
) COMMENT '同一用户对同一业务对象绑定同一文件只允许一次',
    INDEX idx_user_created
(
    user_id,
    created_at
...
(
    16
) NOT NULL COMMENT '会话类型：single-单聊，group-群聊，global-全站特殊群聊',
    name VARCHAR
(
    128
) NULL COMMENT '会话名称（群聊/全站群使用，单聊可为空）',
    avatar VARCHAR
(
    512
) NULL COMMENT '会话头像',
    owner_id BIGINT NULL COMMENT '会话拥有者ID（普通群聊群主）',
    single_pair_key VARCHAR
(
    64
) NULL COMMENT '单聊唯一键，格式建议为 userIdMin:userIdMax',
    is_all_site TINYINT DEFAULT 0 NOT NULL COMMENT '是否全站特殊群聊：0-否，1-是',
    all_site_key VARCHAR
(
    16
) GENERATED ALWAYS AS
(
...
(
    ...
    UNIQUE KEY uk_single_pair
(
    single_pair_key
) COMMENT '单聊会话唯一键',
    UNIQUE KEY uk_all_site_key
(
    all_site_key
) COMMENT '全站特殊群聊唯一键',
...
(
    16
) DEFAULT 'member' NOT NULL COMMENT '成员角色：owner/admin/member',
    join_source VARCHAR
(
    16
) DEFAULT 'manual' NOT NULL COMMENT '入群来源：manual/system',
...
(
    16
) NOT NULL COMMENT '消息类型：text/image/file/system',
...
(
    sender_id,
    client_message_id
) COMMENT '发送人 + 客户端消息ID唯一',
    INDEX idx_conversation_message
(
    conversation_id,
    id
    DESC
) COMMENT '按会话倒序查询消息',
    INDEX idx_sender_message
(
    sender_id,
    id
    DESC
) COMMENT '按发送人倒序查询消息',
    INDEX idx_reply_message
(
    reply_message_id
) COMMENT '按被回复消息查询'
    ) COMMENT '聊天消息表'
...
(
    16
) DEFAULT 'normal' NOT NULL COMMENT '接收类型：normal/mention/system',
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    UNIQUE KEY uk_message_recipient
(
    message_id,
    recipient_user_id
) COMMENT '同一消息对同一接收人只生成一条接收状态',
...
(
    ...
    INDEX idx_conversation_recipient
(
    conversation_id,
    recipient_user_id,
    id
    DESC
) COMMENT '按会话和接收人查询消息状态'
    ) COMMENT '聊天消息接收状态表'
    ENGINE = InnoDB
    DEFAULT CHARSET = utf8mb4
    COLLATE = utf8mb4_unicode_ci;
...
(
    16
) NOT NULL COMMENT '消息类型：image/voice',
...
(
    message_id
) COMMENT '同一消息只保留一条媒体处理任务',
    INDEX idx_status_retry_time
(
    task_status,
    next_retry_at,
    id
) COMMENT '按状态和下次执行时间扫描任务',
    INDEX idx_status_lease_expire
(
    task_status,
    lease_expire_at
) COMMENT '按租约过期时间恢复处理中任务'
    ) COMMENT '聊天附件异步处理任务表'
    ENGINE = InnoDB
    DEFAULT CHARSET = utf8mb4
    COLLATE = utf8mb4_unicode_ci;
...
(
    32
) DEFAULT 'manual' NOT NULL COMMENT '关注来源：manual/recommend/system',
...
(
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP NOT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    UNIQUE KEY uk_follower_following
(
    follower_id,
    following_id
) COMMENT '同一关注关系唯一',
    INDEX idx_follower_status_time
(
    follower_id,
    follow_status,
    follow_time
    ...
    INDEX idx_following_status_time
(
    following_id,
    follow_status,
    follow_time
    ...
    INDEX idx_follow_pair_status
(
    follower_id,
    following_id,
    follow_status
) COMMENT '判断是否已关注/互关',
    CONSTRAINT chk_no_self_follow CHECK
(
    follower_id
    <>
    following_id
...
END IF;
    ...
    SET
@file_reference_columns = (SELECT GROUP_CONCAT(column_name ORDER BY seq_in_index SEPARATOR ',')
        FROM information_schema.statistics
        WHERE table_schema = DATABASE()
...
SET
@ddl = 'ALTER TABLE file_business_info ADD UNIQUE KEY uk_file_reference (file_id, user_id, reference_type, reference_id)';
PREPARE stmt FROM @ddl;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
END IF;
...
VALUES (1790, 1700, '0,1700', '聊天管理', 'M', 'ContentChat', 'chats', 'content/chat/index', NULL, 0, 1, 1, 9,
       ...
       (1792, 1790, '0,1700,1790', '会话状态', 'B', NULL, NULL, NULL, 'content:chat:update', 0, 0, 1, 2, NULL, NULL,
        NOW(), NOW(), NULL),
       (1793, 1700, '0,1700', '关注管理', 'M', 'ContentFollow', 'follows', 'content/follow/index', NULL, 0, 1, 1, 10,
       ...
       (1795, 1793, '0,1700,1793', '关注清理', 'B', NULL, NULL, NULL, 'content:follow:clean', 0, 0, 1, 2, NULL, NULL,
        NOW(), NOW(), NULL) ON DUPLICATE KEY
UPDATE
    parent_id =
VALUES (parent_id), tree_path =
VALUES (tree_path), name =
VALUES (name), type =
VALUES (type), route_name =
VALUES (route_name), route_path =
VALUES (route_path), component =
VALUES (component), perm =
VALUES (perm), always_show =
VALUES (always_show), keep_alive =
VALUES (keep_alive), visible =
VALUES (visible), sort =
VALUES (sort), icon =
VALUES (icon), redirect =
VALUES (redirect), update_time = NOW(), params =
VALUES (params);

INSERT
IGNORE INTO `sys_role_menu` (`role_id`, `menu_id`)
SELECT 1, 1790
FROM DUAL
