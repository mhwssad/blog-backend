...
**当前阶段：业务功能已完成，清爽性检查已完成。已完成 `sys_author_application`、`sys_user` 等级字段、`sys_user_notification_setting`、博客内容侧结构承接、聊天侧结构对齐、AI 侧结构对齐、举报治理侧结构对齐及权限菜单补全，所有表结构与业务代码已对齐。已修复 XML ResultMap type 引用、清理死枚举、规范化 SQL 脚本命名。**

```
已具备:
  ✅ 01_sys.sql 已补用户等级、作者申请、通知偏好、AI、举报治理结构
  ...
  ✅ XML ResultMap type 引用已修复（43 文件）
  ✅ 死枚举已清理（ExperienceResultCode、StorageType.COS 等）
  ✅ SQL 初始化脚本命名已规范化（01_sys.sql）
...
- [x] 不新增与现有表语义重复的平行结构。
- [x] 已明确废弃的字段、表、文档和任务清单及时删除。
- [x] 新增枚举只保留当前阶段会使用的值。
- [x] 初始化脚本只保留当前实际执行入口。
- [x] 文档中不保留”多年历史兼容”的无效说明。
