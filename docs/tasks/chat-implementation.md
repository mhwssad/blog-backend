...
- 群发现：公开普通群支持关键词和分类搜索，私密群、禁用群和已解散群不会进入搜索结果
- 群邀请链接：群主 / 管理员可创建和停用邀请链接，登录用户可凭有效 token 入群
- 主题频道：后台可直接创建和编辑主题频道配置，频道申请通过后也会自动生成主题频道
...
- 入群申请：普通群支持用户主动申请，群主 / 管理员分页查看并审批，通过后创建或恢复成员关系
...
- `chat_group_join_application`
    - 保存普通群主动入群申请
    - `apply_status = 0/1/2/3` 分别表示待审核、已通过、已拒绝、已取消
    - 审核通过后成员 `join_source` 写为 `application`
- `chat_group_invite_link`
    - 保存普通群邀请链接 token、过期时间、使用次数和启停状态
    - `max_use_count = 0` 表示不限制使用次数
    - 凭链接入群后成员 `join_source` 写为 `invite_link`
- 主题频道仍复用 `chat_conversation`
    - `conversation_type = group`
    - `scene_type = topic_channel`
    - `allow_guest_view = 0`
    - `require_join_to_speak = 1`
...
- 创建普通群：
    - 建群前校验系统配置 `chat.group.create.min-level`
    - 建群前校验系统配置 `chat.group.create.max-count`
    - 创建时可写入群简介、公告、分类、可见范围、加入规则、发言等级和成员上限
    - `memberLimit = 0` 表示不限制；创建、邀请和审批入群都会校验人数上限
- 普通群入群申请：
    - 单聊、全站群和不可用群聊不支持入群申请
    - 已是正常成员时不能重复申请
    - 已存在待审核申请时不能重复提交
    - `joinRule = invite_only` 的群聊不接受主动申请
    - 群主和管理员可以审核入群申请，审核通过会创建或恢复申请人的成员关系
- 普通群邀请链接：
    - 群主和管理员可以创建、查询和停用邀请链接
    - 邀请链接支持过期时间和最大使用次数
    - 已停用、已过期或达到使用次数上限的链接不能继续入群
    - 已是正常成员时重复使用链接直接返回成功
    - 通过邀请链接入群仍会校验群人数上限
- 主题频道后台管理：
    - 管理员可直接创建主题频道
    - 管理员可编辑频道名称、简介、公告、分类、可见范围、加入规则、发言等级、人数上限、慢速模式和排序
    - 主题频道启用 / 禁用复用后台会话状态接口
    - 主题频道成员和消息查看复用后台会话成员 / 消息接口
- 普通成员：
    - 无治理权限

当前群公告已使用 `chat_conversation.announcement` 字段存储，`remark` 仅保留备注语义。
...
- 群公告当前使用 `chat_conversation.announcement`，不要再把 `remark` 当成公告字段使用。
